...
#pragma mark -
#pragma mark Error and Logging managament

typedef enum tagAlertType {AlertTypeInfo, AlertTypeWarning, AlertTypeError} AlertType;

- (void)remoteLog:(NSString *)msg type:(AlertType)type {
	NSString *logType;
	switch (type) {
		case AlertTypeInfo:
			logType = @"Information";
	...
	if ([self isInternetAvailable]) {
		...
		NSString *description = [msg  stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
		NSString *moduleName = @"RSSReader";
		int codeLine = 0;
		NSString *requestString = [NSString stringWithFormat:mobileAPILoggingRequest, self.appID, logType, description, moduleName, codeLine];
		requestString = [NSString stringWithFormat:@"%@%@", mobileAPIURL, requestString];
		NSLog(@"REMOTE-LOG: %@", requestString);
		...
		// store logs for future communiation
	}
	
}

- (void)handleAlertView:(NSString *)title 
...
otherButtonTitles:(NSString *)otherBtnTitles {
	...
	[self handleAlertView:title message:msg delegate:delegate cancelButtonTitle:cancelBtnTitle otherButtonTitles:otherBtnTitles type:AlertTypeError];
}

- (void)warning:(NSString *)title 
				message:(NSString *)msg 
			 delegate:(id)delegate
cancelButtonTitle:(NSString *)cancelBtnTitle 
otherButtonTitles:(NSString *)otherBtnTitles {
	
	[self handleAlertView:title message:msg delegate:delegate cancelButtonTitle:cancelBtnTitle otherButtonTitles:otherBtnTitles type:AlertTypeWarning];
}
...
otherButtonTitles:(NSString *)otherBtnTitles {
	...
	[self handleAlertView:title message:msg delegate:delegate cancelButtonTitle:cancelBtnTitle otherButtonTitles:otherBtnTitles type:AlertTypeInfo];
}
...
- (void)applicationDidFinishLaunching:(UIApplication *)application {
	...
	NSLog(@"AppID read from NSUserDefaults: %@", self.appID);
	if (!self.appID || [self.appID isEqualToString:@""])
		[self loadAppID];
	
	[self remoteLog:@"LoggedIn" type:AlertTypeInfo];
	...
	welcomeSound = [[[SoundEffect alloc] initWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"Welcome_eng" ofType:@"wav"]] retain];
...
- (void)applicationWillTerminate:(UIApplication *)application {
	...
	[self remoteLog:@"WillTerminate" type:AlertTypeInfo];
}

- (void)applicationDidBecomeActive:(UIApplication *)application {
	[self remoteLog:@"BecameActive" type:AlertTypeInfo];
}

- (void)applicationDidEnterBackground:(UIApplication *)application {
	[self remoteLog:@"EnteredBackground" type:AlertTypeInfo];
}
...
- (void)warnAboutNoConnectivity {
	NSString *msg;
	if ([self onlyWiFiAllowed])
		msg = @"NoWiFiConnectionAvailable";
	else
		msg = @"NoConnectionAvailable";
	
	[self warning:NSLocalizedString(@"", @"") 
...
- (void)handleError:(NSError *)error {
	...
	[self error:NSLocalizedString(@"LoadingAppIDErrorTitle", @"") message:errorMessage delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
}
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
	...
	if (data) {
		NSString *jsonString = [NSString stringWithCString:data.mutableBytes encoding:NSUTF8StringEncoding];
		NSLog(@"connectionDidFinishLoading with data: %@", jsonString);
