...
@synthesize refreshTimer, newsViewControllers, app;
...
- (void)internetReachabilityChanged:(NSNotification *)note {
	//app = [note object];
	internetAvailable = [self.app isInternetAvailable];
	if (!channelsInitialized) {
		...
		if (internetAvailable) {
			...
			channelsInitialized = YES;
		}
	}
	else if (internetAvailable && [[self.tabBarController selectedViewController] isEqual:self.navigationController]) {
		for (Channel *channel in channels) {
			//[channel reload];
		}
	}
...
- (void)refreshRateChanged:(NSNotification *)note {
	...
	if (self.app.refreshRate > 0) {
		refreshTimer = [[NSTimer timerWithTimeInterval:self.app.refreshRate target:self	selector:@selector(refreshTimerTick:) userInfo:nil repeats:YES] retain];
...
- (void)tableView:(UITableView *)table didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if (!news) {
		...
		news.view.backgroundColor = [UIColor clearColor];
