...
- (void)reload {
	...
	if (oldSavedArray != nil) {
		...
		if (((self.app != nil) && [self.app removeDeadline]) || ((self.app == nil) && [defaults integerForKey:@"removeDeadline"])) {
			for (int j=0; j<[oldSavedArray count]; j++) {
				...
				for (int i=0; i<[[channel articles] count]; i++) {
					...
					if ([[NSDate date] timeIntervalSinceDate:article.stamp] >= [self.app removeDeadline]) {
		...
		for (Channel *channel in savedChannels) {
			for (SavedArticle *article in [channel articles])
